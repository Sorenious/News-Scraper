...
$.getJSON("/articles", function(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    $("#articles").append("<div class='scraped-article' data-id='" + data[i]._id + "'><h2>" + data[i].title + "</h2><p>" + data[i].link + "</p><p>" + data[i].summary + "</p></div>");
  }
});
    ...
    .then(function(data) {
      ...
      if (data.note) {
        for (var i = 0; i < data.note.length; i++) {
          $("#notes-list").append("<div class='note'><h4>" + data.note[i].title + "</h4><p>" + data.note[i].body + "</p></div>");
